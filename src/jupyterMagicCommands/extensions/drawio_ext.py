...
@argument('--base_jupyter_url', type=str, default=os.getenv("JMC_JUPYTER_BASE_URL"), help="Base url of the jupyterlab")
@argument('--password', type=str, default=os.getenv("JMC_JUPYTER_PASSWORD"), help="Password of the jupyterlab")
@argument("--logLevel", type=parse_logLevel, default="ERROR", help="Set the log level. The valid log levels are: DEBUG, INFO, WARNING, ERROR, CRITICAL.")
@register_line_magic
def drawio(line):
    args = parse_argstring(drawio, line)
    if not args.base_jupyter_url:
        raise Exception("You need to set the base url by --base_jupyter_url option or JMC_JUPYTER_BASE_URL environment variable")
    ...
    jupyter_client = JupyterClient(
        base_url=args.base_jupyter_url, 
